...
export function ProducePcbModal({ isOpen = false, ...rest }) {
  ...
  const canProducePcb = (parts, pcb = null) => {
    ...
    if (pcb?.pcbId > 0) {
      const pcbParts = _.filter(parts, part => part.pcbId === pcb.pcbId);
      const pcbCount = getProduciblePcbCount(pcbParts, pcb);
      // console.debug(`canProducePcb name: ${pcb.name} pcbCount: ${pcbCount.count} quantity: ${formQuantity} result: ${pcbCount.count >= formQuantity}`, pcbParts, parts);
  ...
  const createPcbOptions = (project) => {
    const options = [
      { key: -1, text: t('comp.producePcbModal.options.all', "All"), description: t('comp.producePcbModal.options.allDescription', "Produce the entire BOM"), value: -1, icon: getProducibleBomCount(project.parts, project.pcbs) === 0 && "warning circle", title: getProducibleBomCount(project.parts, project.pcbs) === 0 ? t('comp.producePcbModal.options.notEnoughParts', "Does not have enough parts to produce") : '', disabled: getProducibleBomCount(project.parts, project.pcbs).count === 0 },
      { key: 0, text: t('comp.producePcbModal.options.unassociated', "Unassociated"), description: t('comp.producePcbModal.options.unassociatedDescription', "Produce parts not associated to a PCB"), value: 0, icon: !canProducePcb(project.parts) && "warning circle", title: !canProducePcb(project.parts) ? t('comp.producePcbModal.options.notEnoughParts', "Does not have enough parts to produce") : '', disabled: getProducibleUnassociatedCount(project.parts) === 0 },
    ];

    if (project?.pcbs?.length > 0) {
      for (let i = 0; i < project.pcbs.length; i++) {
        options.push({ key: i + 1, text: project.pcbs[i].name, description: project.pcbs[i].description, value: project.pcbs[i].pcbId, icon: !canProducePcb(project.parts, project.pcbs[i]) && "warning circle", title: !canProducePcb(project.parts, project.pcbs[i]) ? t('comp.producePcbModal.options.notEnoughParts', "Does not have enough parts to produce") : '', disabled: !canProducePcb(project.parts, project.pcbs[i]) });
      }
    }
    return options;
  };
  ...
  const handleChange = (e, control) => {
    ...
    for (let i = 0; i < control.value.length; i++) {
      ...
      const targetPcb = _.find(project.pcbs, pcb => pcb.pcbId === control.value[i]);
  ...
  const handleSerialNumberChange = (e, control) => {
    const p = _.find(project.pcbs, pcb => pcb.pcbId === control.id);
  ...
  const handleSubmit = (e) => {
    if (rest.onSubmit) {
      let pcbsToProcess = _.filter(project.pcbs, pcb => form.pcbs.includes(pcb.pcbId));
      if (form.pcbs.includes(-1))
        pcbsToProcess = project.pcbs;

      if (project.parts.length === 0) {
        toast.error("No parts in project to produce.");
        return;
      }
      ...
      for (let i = 0; i < pcbsToProcess.length; i++) {
        if (canProducePcb(project.parts, pcbsToProcess[i])) {
          validPcbsToProcess.push(pcbsToProcess[i]);
        } else {
          console.debug('cannot produce pcb', pcbsToProcess[i]);
        }
      }

      // only process unassociated if parts are available
      let processUnassociated = false;
      if ((form.pcbs.includes(0) || form.pcbs.includes(-1)) && canProducePcb(project.parts))
      ...
      console.debug('producing', request);
      if (request.pcbs.length === 0 && !request.unassociated) {
        toast.error("No valid PCBs selected to produce.");
        return;
      }
      rest.onSubmit(e, request);
    } else {
  ...
  const getPcbsToDisplay = useCallback((project) => {
    if (!project) return [];
    let pcbsToDisplay = [..._.filter(project.pcbs, pcb => form.pcbs.includes(pcb.pcbId))];
    if (form.pcbs.includes(-1)) {
      // include all pcbs
                    ...
                    {getPcbsToDisplay(project).map((pcb, key) => {
                      ...
                      return (
                        <Table.Row key={key} className={!canProduce ? "disabled" : ""}>
                          ...
                          <Table.Cell textAlign="center">{pcb?.pcbId > 0 ? _.filter(project.parts, part => part.pcbId === pcb.pcbId).length : _.filter(project.parts, part => part.pcbId === null).length}</Table.Cell>
                          <Table.Cell textAlign="center">{getTotalPartsOutOfStock(pcb?.pcbId > 0 ? pcb : null)}</Table.Cell>
                          <Table.Cell>{!canProduce && (pcb?.pcbId > 0 ? _.filter(project.parts, part => part.pcbId === pcb.pcbId).length : _.filter(project.parts, part => part.pcbId === null).length > 0) && <span><Icon name="warning circle" color="red" /> {t('message.notEnoughParts', "Not enough parts")}</span>}</Table.Cell>
                        </Table.Row>
                      );
                    })}
                  </Table.Body>
