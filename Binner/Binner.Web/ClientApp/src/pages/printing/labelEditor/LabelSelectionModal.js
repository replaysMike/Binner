      ...
      if (response.responseObject.ok) {
        ...
        const newLabelOptions = [
          ...
          ...data.map((item, key) => ({
            ...
            text: item.name,
            value: item.labelId,
            icon: item.isPartLabelTemplate ? "checkmark" : "",
            description: <span className="label-option">{item.isPartLabelTemplate ? <span className="default">Default Part Label</span> : <></>} {!item.isEditable && <span className="readonly">Readonly</span>}</span>
          }))
        ];
