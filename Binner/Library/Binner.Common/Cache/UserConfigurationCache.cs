...
namespace Binner.Common.Integrations
{
    ...
    {
        private readonly Dictionary<long, UserConfigurationCacheStore> _configCache = new Dictionary<long, UserConfigurationCacheStore>();
        private readonly SemaphoreSlim _lock = new SemaphoreSlim(1, 1);

        public UserConfigurationCache() { }

        public UserConfigurationCache(Dictionary<long, UserConfigurationCacheStore> credentials)
        {
            ...
            where TConfig : class
        {
            ...
            try
            {
                TConfig? config;
                if (!_configCache.ContainsKey(key))
                    _configCache.Add(key, new UserConfigurationCacheStore());
