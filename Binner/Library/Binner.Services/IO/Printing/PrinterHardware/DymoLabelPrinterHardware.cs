...
namespace Binner.Services.IO.Printing
{
    ...
    public class DymoLabelPrinterHardware : ILabelPrinterHardware
    {
        ...
        public Image<Rgba32> PrintLabel(ICollection<LineConfiguration> lines, PrinterOptions options)
        {
            ...
            if (options is null) throw new ArgumentNullException(nameof(options));
            var (image, labelProperties) = CreatePrinterImage(options);
            DrawLabelFromLines(image, labelProperties, lines, _paperRect, options);
        ...
        private void DrawBoundsDebug(Image<Rgba32> image, LabelDefinition labelProperties)
        {
            // draw rectangle around bounds
            image.Mutate(c => c.Draw(Pens.Solid(Color.Red, 1), new RectangleF(0, 0, _paperRect.Width - 1, _paperRect.Height - 1)));
        }
        ...
        private (Image<Rgba32>, LabelDefinition labelProperties) CreatePrinterImage(PrinterOptions options)
        {
            ...
            if (string.IsNullOrWhiteSpace(labelName))
                labelName = _printerSettings.PartLabelName;
            ...
            printerImage.Mutate(c => c.Fill(Color.White));

            return (printerImage, labelProperties);
        }

        private void DrawLabelFromLines(Image<Rgba32> image, LabelDefinition labelProperties, ICollection<LineConfiguration> lines, Rectangle paperRect, PrinterOptions options)
        {
            ...
            {
                prevLine = DrawLine(image, labelProperties, prevLine, null, line.Content, line, paperRect, margins, out var bounds);
                if (options.ShowDiagnostic) image.Mutate(c => c.DrawPolygon(Pens.Solid(Color.Blue, 2), new PointF(bounds.X, bounds.Y), new PointF(bounds.X + bounds.Width, bounds.Y), new PointF(bounds.X + bounds.Width, bounds.Y + bounds.Height), new PointF(bounds.X, bounds.Y + bounds.Height)));
            }
        }
        ...
        private void DrawLabelFromContent(Image<Rgba32> image, LabelDefinition labelProperties, LabelContent content, Rectangle paperRect)
        {
            ...
            if (!string.IsNullOrEmpty(_printerSettings.PartLabelTemplate.Identifier?.Content) && _printerSettings.PartLabelTemplate.Identifier?.Position == LabelPosition.Right)
                rightMargin = 5;
            if (!string.IsNullOrEmpty(_printerSettings.PartLabelTemplate.Identifier?.Content) && _printerSettings.PartLabelTemplate.Identifier?.Position == LabelPosition.Left)
                leftMargin = 5;
            var margins = new Margin(leftMargin, rightMargin, 0, 0);

            // process template values
            content.Line1 = content.Line1 ?? ReplaceTemplate(content.Part, _printerSettings.PartLabelTemplate.Line1);
            content.Line2 = content.Line2 ?? ReplaceTemplate(content.Part, _printerSettings.PartLabelTemplate.Line2);
            content.Line3 = content.Line3 ?? ReplaceTemplate(content.Part, _printerSettings.PartLabelTemplate.Line3);
            content.Line4 = content.Line4 ?? ReplaceTemplate(content.Part, _printerSettings.PartLabelTemplate.Line4);
            content.Identifier = content.Identifier ?? ReplaceTemplate(content.Part, _printerSettings.PartLabelTemplate.Identifier);
            content.Identifier2 = content.Identifier2 ?? ReplaceTemplate(content.Part, _printerSettings.PartLabelTemplate.Identifier2);

            // merge any adjacent template lines together
            MergeLines(_printerSettings.PartLabelTemplate, content, paperRect, margins);
            var index = _printerSettings.PartLabelTemplate.Line1?.Label ?? 1;
            var line0Position = _labelStart[index - 1];
            var line1Position = DrawLine(image, labelProperties, line0Position, content.Part, content.Line1, _printerSettings.PartLabelTemplate.Line1, paperRect, margins, out var bounds);
            var line2Position = DrawLine(image, labelProperties, line1Position, content.Part, content.Line2, _printerSettings.PartLabelTemplate?.Line2, paperRect, margins, out bounds);
            var line3Position = DrawLine(image, labelProperties, line2Position, content.Part, content.Line3, _printerSettings.PartLabelTemplate?.Line3, paperRect, margins, out bounds);
            var line4Position = DrawLine(image, labelProperties, line3Position, content.Part, content.Line4, _printerSettings.PartLabelTemplate?.Line4, paperRect, margins, out bounds);

            var identifierPosition = DrawLine(image, labelProperties, line4Position, content.Part, content.Identifier, _printerSettings.PartLabelTemplate?.Identifier, paperRect, margins, out bounds);
            var identifierPosition2 = DrawLine(image, labelProperties, identifierPosition, content.Part, content.Identifier2, _printerSettings.PartLabelTemplate?.Identifier2, paperRect, margins, out bounds);
        }
        ...
        {
            var fontFirstLine = CreateFont(_printerSettings.PartLabelTemplate.Line2, firstLine, paperRect);
            var fontSecondLine = CreateFont(_printerSettings.PartLabelTemplate.Line3, secondLine, paperRect);
        ...
        private PointF DrawLine(Image<Rgba32> image, LabelDefinition labelProperties, PointF lineOffset, object? part, string? text, LineConfiguration? template, Rectangle paperRect, Margin margins, out RectangleF bounds)
        {
            if (string.IsNullOrWhiteSpace(text) || template == null)
            {
                bounds = new RectangleF(lineOffset.X, lineOffset.Y, 0, 0);
            ...
            if (template.Barcode == true)
            {
                x = template.Margin.Left;
                y += 12;
                var barcodeWidth = paperRect.Width - (template.Margin.Left * 2);
                var barcodeHeight = paperRect.Height;
                DrawBarcode128(image, fontColor, Color.White, text, new Rectangle((int)x, (int)y, barcodeWidth, barcodeHeight), (int)((template.FontSize ?? 6) * 10f));
                bounds = new RectangleF(x, y, barcodeWidth, barcodeHeight);
            }
            else
            {
                ...
                if (template.Rotate > 0)
                {
                    ...
                    var builder = new AffineTransformBuilder()
                        .AppendRotationDegrees(_printerSettings.PartLabelTemplate?.Identifier?.Rotate ?? 0)
                ...
                bounds = new RectangleF(x, y, textBounds.Width, textBounds.Height);
            }
        ...
        private void DrawBarcode128(Image<Rgba32> image, Color foregroundColor, Color backgroundColor, string encodeValue, Rectangle rect, int barcodeHeight)
        {
            ...
            if (rect.X > 0)
                width = rect.Width;
            var generatedBarcodeImage = _barcodeGenerator.GenerateBarcode(encodeValue, foregroundColor, backgroundColor, width, Math.Max(barcodeHeight, rect.Height));
            try
            {
                //image.Mutate(c => c.DrawImage(generatedBarcodeImage, new Rectangle(rect.X, rect.Y, rect.Width - 120, barcodeHeight), graphicsOptions));
                image.Mutate(c => c.DrawImage(generatedBarcodeImage, new Point(rect.X, rect.Y), graphicsOptions));
