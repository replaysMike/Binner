...
namespace Binner.Services.Integrations
{
    public sealed class DigikeyV4Api : BaseDigikeyApi, IDigikeyApi
    {
        ...
        {
            if (startDate == null) startDate = DateTime.Now.Subtract(TimeSpan.FromDays(180));
            ...
            try
            {
                ...
                var uri = Url.Combine(_configuration.ApiUrl, $"orderstatus/v4/orders?startDate={startDate?.ToString("yyyy-MM-dd")}&endDate={endDate?.ToString("yyyy-MM-dd")}&pageNumber={pageNumber}&pageSize={pageSize}");
        ...
        public async Task<IApiResponse> GetOrderAsync(OAuthAuthorization authenticationResponse, string orderId)
        {
            ...
            try
            {
                ...
                var responseJson = await response.Content.ReadAsStringAsync();
                _logger.LogTrace($"{uri}: {responseJson}");
                var results = JsonConvert.DeserializeObject<SalesOrder>(responseJson, _serializerSettings) ?? new();
                return new ApiResponse(results, nameof(DigikeyApi));
            }
            catch (Exception)
            {
