...
namespace Binner.Services.Integrations
{
    public class PartInformationProvider : IPartInformationProvider
    {
        ...
        public PartInformationProvider(IIntegrationApiFactory integrationApiFactory, ILogger logger, WebHostServiceConfiguration configuration, IUserConfigurationService userConfigurationService, IPartTypeDetection<CommonPart> partTypeDetection)
        {
        ...
        private async Task<ProcessingContext> ProcessResponseAsync(Type provider, IIntegrationApi api, ProcessingContext context, int maxResults)
        {
            ...
            var providerImplementations = new Dictionary<Type, Func<Task>>()
            {
                { typeof(SwarmApi), async () =>
                    {
                        var processor = new SwarmPartInfoResponseProcessor(_logger, _configuration, userConfiguration, _partTypeDetection, 10, maxResults);
                        await processor.ExecuteAsync(api, context);
                    }
                },
                { typeof(DigikeyApi), async () =>
                    {
                        var processor = new DigiKeyPartInfoResponseProcessor(_logger, _configuration, userConfiguration, _partTypeDetection, 20, maxResults);
                        await processor.ExecuteAsync(api, context);
                    }
                },
                { typeof(MouserApi), async () =>
                    {
                        var processor = new MouserPartInfoResponseProcessor(_logger, _configuration, userConfiguration, _partTypeDetection, 30, maxResults);
                        await processor.ExecuteAsync(api, context);
                    }
                },
                { typeof(ArrowApi), async () =>
                    {
                        var processor = new ArrowPartInfoResponseProcessor(_logger, _configuration, userConfiguration, _partTypeDetection, 40, maxResults);
                        await processor.ExecuteAsync(api, context);
                    }
                },
                { typeof(NexarApi), async () =>
                    {
                        var processor = new NexarPartInfoResponseProcessor(_logger, _configuration, userConfiguration, _partTypeDetection, 50, maxResults);
                        await processor.ExecuteAsync(api, context);
                    }
                },
                { typeof(TmeApi), async () =>
                    {
                        var processor = new TmePartInfoResponseProcessor(_logger, _configuration, userConfiguration, _partTypeDetection, 60, maxResults);
                        await processor.ExecuteAsync(api, context);
                    }
                },
                { typeof(Element14Api), async () =>
                    {
                        var processor = new Element14PartInfoResponseProcessor(_logger, _configuration, _partTypeDetection, 60);
                        await processor.ExecuteAsync(api, context);
                    }
                },
            };
