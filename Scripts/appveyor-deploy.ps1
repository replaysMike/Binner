...
$sw = [Diagnostics.Stopwatch]::StartNew()
  ...
  docker build --no-cache --progress=plain --build-arg VERSION=$env:APPVEYOR_BUILD_VERSION --build-arg PORT=8090 -t "$env:DOCKER_USERNAME/binner:$versionTag" -t "$env:DOCKER_USERNAME/binner:latest" --build-arg VERSION=$env:APPVEYOR_BUILD_VERSION .
$sw.Stop()
